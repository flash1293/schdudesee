...
class Handler(SimpleHTTPRequestHandler):
    ...
    def serve_organizers(self):
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        rows = c.execute("SELECT DISTINCT organizer FROM curated_events WHERE organizer IS NOT NULL AND organizer != '' AND tags != 'blocked' ORDER BY organizer").fetchall()
        conn.close()
        orgs = [html.unescape(r[0]) for r in rows]
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.send_header("Access-Control-Allow-Origin", "*")
        self.end_headers()
        self.wfile.write(json.dumps(orgs, ensure_ascii=False).encode())
